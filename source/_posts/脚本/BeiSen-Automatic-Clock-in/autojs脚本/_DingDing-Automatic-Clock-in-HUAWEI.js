...
const FIF_MIN = 15 * 60 * 1000 // 等待时间：0.25h
const THI_MIN = 30 * 60 * 1000 // 等待时间：0.5h
const FOR_MIN = 45 * 60 * 1000 // 等待时间：0.75h
...
function notificationHandler(n) {
    ...
    switch(text) {
        ...
        case "日志": // 监听文本为 "日志" 的通知
            // threads.shutDownAll()
            threads.start(function(){
                sendEmail("获取日志", "通过邮件附件发送日志文件", globalLogFilePath)
            })
            break;
 ...
 */
 function closeColck(){
    ...
    if (null == btn){
        console.log("未找到关闭按钮，无法关闭考勤机...")
        return;
    }

    console.log("点击关闭按钮，关闭考勤机...")
    btn.click()
    sleep(3000)
}
