...
while(recursiveCard){
    ...
    while(hour != 8 && hour != 22){
        console.warn("睡眠 5 分钟")
        sleep(UPPER_BOUND)
        hour = new Date().getHours()
    }

    var randomTime = random(ONE_MIN, AN_HOUR)
    toastLog(Math.floor(randomTime / 1000) + "秒后启动钉钉...")
    sleep(randomTime)
    doClock()
    ...
    switch(text) {
        ...
        case "开启循环": // 每天循环打卡 早八晚十
            ...
            threads.start(function(){
                switch(DEFAULT_MESSAGE_DELIVER) {
                    case PUSH_METHOD.QQ:
                        sendQQMsg("开启每天循环打卡功能")
                       break;
                    case PUSH_METHOD.Email:
                        sendEmail("开启每天循环打卡功能", null)
                       break;
                    case PUSH_METHOD.ServerChan:
                        sendServerChan("开启每天循环打卡功能")
                       break;
                    case PUSH_METHOD.PushDeer:
                        sendPushDeer("开启每天循环打卡功能")
                       break;
                }
            })
            break;
